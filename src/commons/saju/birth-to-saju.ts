...
async function convertBirthtimeToSaju(
  ...
  const time = memberDto.time;
  const birthday =
    time >= '23:30' && time <= '23:59'
      ? moment(memberDto.birthday).add(1, 'days').format('YYYY-MM-DD')
      : moment(memberDto.birthday).format('YYYY-MM-DD');

  const birthTime = time ? time : '12:00'; //시간 미 입력시 대운수 계산을 위핸 디폴트 12:00로 처리
  const birthType = memberDto.birthdayType;
  const gender = memberDto.gender;

  const condition =
    birthType === 'SOLAR' ? { solarDate: birthday } : { lunarDate: birthday };
  const samju = await Manse.findOne(condition);
  const solarDatetime = moment(birthday + ' ' + birthTime);
  const solarDatetimeFormat = solarDatetime.format('YYYY-MM-DD HH:mm:ss');
  ...
  //순행, 역행 판단
  const minusPlus = await CommonData.getMinusPlus()[samju.yearSky];
  //남양음녀 순행, 남음여양 역행
  let direction = false;
  if (
  ...
  const seasonStartTime = moment(manse.seasonStartTime);
  ...
  const bigFortuneStart = solarDatetime
  ...
  let timeSky = null;
  let timeGround = null;
  const daySky = samju.daySky;

  if (time) {
    let index = null;
    const timeJuData = CommonData.getTimeJuData(); //시간 범위
    const timeJuData2 = CommonData.getTimeJuData2(); //일간 및 시간에 따른 시주
